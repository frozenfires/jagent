...
@TaskExecutors(value = "atmcUpdateAct")
public class RDPUpdateExecutor implements TaskExecutor {

//	private EventListener rdpStartListener = new EventListener() {
//		@Override
//		public void event(Event event) {
//			try {
//				RDPUpdateExecutor.this.rdpStartup(event);
//			} catch (Exception e) {
//				log.error("RDP升级监听处理失败", e);
//			}
//		}
//
//		@Override
...
//	};
	...
	private final static Logger log = LoggerFactory.getLogger(RDPUpdateExecutor.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		if (isRDPUpdate(task)) {
			...
			if(agentStart!=null){
				log.info("RDP重启后升级操作，正在执行--------------");
				rdpStartup(serverTask,context);
				return;
			}
			log.info("RDP升级开始，开始时间："+TimeUtil.getNowDateTime()+",任务报文："+task.getTaskData());
				...
				try {
					...
					while(flag){
						try{
							...
							try {
								log.info("rdp升级命令发送失败，1分钟后再次连接rdp");
								Thread.sleep(60000);
							...
							if(isTimeout(serverTask.getExectime(), serverTask.getValid())){
								flag = false;
								log.info("rdp升级命令发送失败，已超时，不再发送");
								throw new JobExecutionException("rdp升级命令发送失败，已超时");
							}
							continue;
						}
						break;
					...
					throw new JobExecutionException("升级失败", e);
				}
				clearCacheFile(serverTask);
	 ...
	 * @return
	 */
	public boolean isRDPUpdate(Task task) {
		if (task != null && task instanceof ServerTask) {
			ServerTask serverTask = (ServerTask) task;
			if ("atmcUpdateAct".equals(serverTask.getTasktype())) {
				return true;
			}
		}
		return false;
	...
	private boolean isSuccessUpdate(ServerTask serverTask) {
		Boolean flag = true;
		// 判断rdp版本
		if (Config.get(Config.NEW_SOF_VER).equals(Config.get(Config.OLD_SOF_VER))) {
			log.info("RDP新版本与旧版本相同，升级失败");
			return false;
		}
		log.info("*******************RDP版本符合***********************");
		// 判断时间
