 ...
 * @author: dingfan
 */
@TaskExecutors(value = "restartAct")
public class SYRestartExecutor implements TaskExecutor {

	private final static Logger log = LoggerFactory.getLogger(SYRestartExecutor.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException{
		...
		if (isSYRestart(task)) {
			...
			log.info("开始向rdp发送系统重启报文");
			boolean flag = true;
			while (flag) {
			    try {
					log.info("向rdp发送系统重启报文");
					String result = RDPMessageSend.send(new RdpEncoder().encode(rdptask));
				} catch (ConnectException e) {
					try {
						log.info("系统重启报文发送失败，1分钟后再次连接rdp");
						Thread.sleep(10000);	
						if (isTimeout(serverTask.getExectime(), serverTask.getValid())) {
							flag = false;
							log.info("rdp系统重启报文发送失败，已超时，不再发送");
							throw new JobExecutionException("系统重启报文发送失败", e);
						}
					} catch (Throwable e1) {
						log.error(null, e);
					}
	...
	public boolean isSYRestart(Task task) {
		if (task != null && task instanceof ServerTask) {
			ServerTask serverTask = (ServerTask) task;
			if ("restartAct".equals(serverTask.getTasktype())) {
				return true;
			}
		}
		return false;
