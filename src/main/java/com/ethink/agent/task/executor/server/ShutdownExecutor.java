 ...
 * @author: dingfan
 */
@TaskExecutors(value = "ShutdownAct")
public class ShutdownExecutor implements TaskExecutor {

	private final static Logger log = LoggerFactory.getLogger(ShutdownExecutor.class);
	...
	public boolean isSYRestart(Task task) {
		if (task != null && task instanceof ServerTask) {
			ServerTask serverTask = (ServerTask) task;
			if ("ShutdownAct".equals(serverTask.getTasktype())) {
				return true;
			}
		}
		return false;
