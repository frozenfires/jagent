 ...
 * @author: dingfan
 */
@TaskExecutors(value = "RestoreAct")
public class RestoreActExecutor implements TaskExecutor {

	private final static Logger log = LoggerFactory.getLogger(RestoreActExecutor.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException{
		...
		if (isSYRestart(task)) {
			...
			log.info("开始向rdp发送恢复ATM报文");
			boolean flag = true;
			while (flag) {
			    try {
					log.info("向rdp发送恢复ATM报文");
					String result = RDPMessageSend.send(new RdpEncoder().encode(rdptask));
				} catch (ConnectException e) {
					try {
						log.info("恢复ATM报文发送失败，1分钟后再次连接rdp");
						Thread.sleep(10000);	
						if (isTimeout(serverTask.getExectime(), serverTask.getValid())) {
							flag = false;
							log.info("rdp恢复ATM报文发送失败，已超时，不再发送");
							throw new JobExecutionException("恢复ATM报文发送失败", e);
						}
					} catch (Throwable e1) {
						log.error(null, e);
					}
	...
	public boolean isSYRestart(Task task) {
		if (task != null && task instanceof ServerTask) {
			ServerTask serverTask = (ServerTask) task;
			if ("RestoreAct".equals(serverTask.getTasktype())) {
				return true;
			}
		}
		return false;
